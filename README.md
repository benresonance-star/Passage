...
- **Home Dashboard**: A central landing page with a personalised greeting, a "Recall" button for spaced repetition (based on the shortest interval of memorised sections, loading all memorised content into a combined view), a "Learn Next" button for seamless progression, and a collapsible Library list.
...
- **Study Backing Library**: Practice / Abide now uses a global instrumental backing-track library (`Presence`, `Stillness`) that is independent of the active verse or chunk. The player appears in Abide whenever the backing library exists, while Song entry still preserves the text-dimming rehearsal flow.
