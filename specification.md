...
- **Top Actions** (collapsible pill, same pattern as Chapter page):
    ...
    - **Group** (Users icon): Navigates to `/group`.
    - **Info** (Info icon): Opens the 7-step meditative memorisation guide.
    - **Settings** (Settings icon): Toggles the expansion of the action pill.
...
- **Soak Intro**: A 3-second introductory screen displaying Psalm 46:10 ("Be still and know that I am God") with a fade-in/fade-out animation before the meditation begins. Styled to match the app's main meditation sequence (light weight, italic, Cormorant Garamond).
...
- **Multi-track Support**: Soak can switch among the merged library through the expanded selector, and only the selected track is loaded. Persisted track selection no longer overrides a fresh user choice.
...
- **Looping**: `useChunkAudio()` now exposes `isLooping` and `toggleLoop()`, wiring the minimalist repeat button directly to the underlying `HTMLAudioElement.loop` flag so the active track can repeat in both Study and Soak.
